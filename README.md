...
- **i18n Support**: Seamless toggle between multiple languages.
...
- **持久化備註**：可為檔案或資料夾添加註解，自動儲存於瀏覽器本地空間。
- **多語系支援**：支援中、英、日、韓介面切換。
- **專業級 PDF 輸出**：
    - 產出專業工程風格結構報告。
    - **SVG 實體連線**：樹狀結構線條採用實體 SVG 繪製，不受「背景圖形」設定影響。
    - **真文字 PDF**：輸出內容為真實文字，支援搜尋與複製。

---

### 🌐 主な機能 (日本語)
- **100% 純ウェブ対応**: インストール不要。ブラウザから直接ローカルディレクトリにアクセス。
- **永続的なメモ**: ファイルやフォルダに注釈を追加でき、ブラウザに自動保存されます。
- **プロフェッショナルな PDF 出力**: 
    - 構造化されたレポートを生成。
    - **SVG ツリーライン**: 背景グラフィックの設定に関わらず、ツリー構造が表示されます。

---

### 🌐 주요 기능 (한국어)
- **100% 순수 웹 기반**: 설치가 필요 없습니다. 브라우저에서 직접 로컬 디렉토리에 액세스합니다.
- **영구 주석**: 파일이나 폴더에 메모를 추가할 수 있으며 브라우저에 자동 저장됩니다.
- **전문적인 PDF 내보내기**: 
    - 엔ジニアリング 스타일의 구조화된 보고서를 생성합니다.
    - **SVG 트리 라인**: 배경 그래픽 설정과 관계없이 구조 선이 명확하게 표시됩니다.
